 ...
 *   --extract --no-cache <url>           → Extract without caching to disk
...
async function extractFromUrl(url, mode = 'all', cacheEnabled = true) {
  ...
  if (cacheEnabled && cache[cacheKey] && (Date.now() - cache[cacheKey].timestamp < CACHE_TTL)) {
    ...
    // Cache (opt-in: warn before storing)
    if (cacheEnabled) {
      console.warn(`[smart-scraper] ⚠️  Caching page content to disk: ${CACHE_FILE}`);
      console.warn(`[smart-scraper]    Page content may contain sensitive data. Use --no-cache to skip.`);
...
(async () => {
  switch (cliMode) {
    case 'extract': {
      const url = findArg();
      if (!url) {
        console.log('Usage: smart-scraper.js --extract [--no-cache] <url>');
      } else {
        await extractFromUrl(url, searchQuery || 'all', !noCache);
      }
      break;
    }
    case 'parse': {
